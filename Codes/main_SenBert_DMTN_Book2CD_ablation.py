...
if __name__=="__main__":
	# ------PREPARE DATASET------#
	source_data_list, target_data_list, kg_data_list = torch.load("Data/formatData_Book2CD_SenBert_V1T2.pt")

	# ------PREPARE dataloader for train, vali, test datasets------#
	train_dataset = data_utils_BertKG_NCF_CDR.DMTN_Data(
		source_data_list, target_data_list, "Train", "source_target", args.data_version)
	...
	for ii in range(K_folds):
		...
		for epoch in range(args.epochs):
			...
			if MAE_source_vali_epoch > min_mae_vali_source and MAE_target_vali_epoch > min_mae_vali_target:
				print('Iter %d, Para: kge_weight:%f, l2_weight:%f, batch_size: %d, learning rate: %f. \n Target RMSE: %f, MAE: %f.\t Source RMSE: %f, MAE: %f.' % \
	...
	torch.save(model.state_dict(), "Data/save_DMTN_epoch_Book2CD_" + str(args.lr) + str(args.model) + args.ablation_model + args.data_version + '.pth')
	...
	with open("Data/save_DMTN_epoch_Book2CD_" + str(args.lr) + '_' +str(args.batch_size) + str(args.model) + args.ablation_model + args.data_version +'.txt', 'w') as f:
		for ii in range(K_folds):
